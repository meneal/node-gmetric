(function() {
  ...
  var Gmetric = (function(){
    ...
    Gmetric.prototype.send_packet = function(host, port, packet, count){
    ...
    //socket.close();
    //if(killflag == true){
        //console.log('kill flag was true');
      //socket.close();
    //}
      });
    };
    ...
    Gmetric.prototype.send = function(host, port, metric){
      ...
      var socket2 = require('dgram').createSocket('udp4');
      socket.send(packet.meta, 0, packet.meta.length, port, host, function(err, bytes){
        if(err)
      console.log(err);
        socket.close();
    });

      socket2.send(packet.data, 0, packet.data.length, port, host, function(err, bytes){
        if(err)
      console.log(err);
        socket2.close();
    });

      //this.send_packet(host, port, packet.meta, count);
      //this.send_packet(host, port, packet.data, count);
    };
